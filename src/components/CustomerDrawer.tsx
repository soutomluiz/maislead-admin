...
export function CustomerDrawer({ client, onClose }: { client: Client; onClose: () => void }) {
      ...
      >
        ...
        <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {atRisk && (
            <div style={{ display: "flex", alignItems: "center", gap: 11, background: "#fff4f5", border: "1px solid #fbd5da", borderRadius: 14, padding: "13px 16px" }}>
              ...
              <div style={{ fontSize: 12.5, color: "#9f1239" }}>
                <b>Risco de churn.</b> Baixo engajamento — recomendável contato proativo.
              </div>
            </div>
          )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <Action variant="primary">Enviar mensagem</Action>
            <Action>Ver assinatura</Action>
            <Action>Entrar como cliente</Action>
            <Action variant="danger">Suspender conta</Action>
          </div>
        </div>
      </div>
    </div>
